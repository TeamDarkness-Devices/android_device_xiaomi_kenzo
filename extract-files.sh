...
if [ -n "$SETUP" ]; then
    ...
    setup_vendor "$DEVICE_COMMON" "$VENDOR" "$AOS_ROOT" true false
    "$MY_DIR"/setup-makefiles.sh false

    if [ -s "$MY_DIR"/../$DEVICE/proprietary-files.txt ]; then
        # Initalize the helper for device
        setup_vendor "$DEVICE" "$VENDOR" "$AOS_ROOT" false false
        "$MY_DIR"/setup-makefiles.sh false
    fi
else
    # Initialize the helper for common
    setup_vendor "$DEVICE_COMMON" "$VENDOR" "$AOS_ROOT" true "$CLEANUP"

    extract "$MY_DIR"/proprietary-files.txt "$SRC"

    if [ -s "$MY_DIR"/../$DEVICE/proprietary-files.txt ]; then
        # Reinitialize the helper for device
        setup_vendor "$DEVICE" "$VENDOR" "$AOS_ROOT" false "$CLEANUP"
